...
DROP TABLE IF EXISTS `tbl_banco` CASCADE;
...
CREATE TABLE `tbl_banco` (
  ...
  `contabilidad` int(11) NOT NULL,
  `cuenta` varchar(20) NOT NULL,
  `banco_nombre` tinytext,
  `banco_cuenta` tinytext,
  `cheque_prefijo` varchar(10) DEFAULT NULL,
  `cheque_numero` int(11) DEFAULT NULL,
  `cheque_lugares` int(11) DEFAULT NULL,
  `cheque_idioma` tinytext,
  `cheque_operado` tinytext,
  `cheque_revisado` tinytext,
  `cheque_autorizado` tinytext,
  `avisar_sobregiro` tinyint(1) DEFAULT NULL,
  `concil_tipo` varchar(3) DEFAULT NULL,
  `concil_operado_nombre` tinytext,
  `concil_operado_cargo` tinytext,
  `concil_revisado_nombre` tinytext,
  `concil_revisado_cargo` tinytext,
  `concil_autorizado_nombre` tinytext,
  `concil_autorizado_cargo` tinytext,
  PRIMARY KEY (`id`,`moneda`,`contabilidad`,`cuenta`),
  KEY `fk_tbl_banco_tbl_moneda1_idx` (`moneda`),
  KEY `fk_tbl_banco_tbl_contabilidad1_idx` (`contabilidad`),
  KEY `fk_tbl_banco_tbl_cuenta1_idx` (`cuenta`),
  CONSTRAINT `fk_tbl_banco_tbl_contabilidad1` FOREIGN KEY (`contabilidad`) REFERENCES `tbl_contabilidad` (`id`) ON UPDATE CASCADE,
  CONSTRAINT `fk_tbl_banco_tbl_cuenta1` FOREIGN KEY (`cuenta`) REFERENCES `tbl_cuenta` (`id`) ON UPDATE CASCADE,
  CONSTRAINT `fk_tbl_banco_tbl_moneda1` FOREIGN KEY (`moneda`) REFERENCES `tbl_moneda` (`id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;


DROP TABLE IF EXISTS `tbl_beneficiario` CASCADE;
...
CREATE TABLE `tbl_beneficiario` (
  ...
  KEY `fk_tbl_beneficiario_tbl_banco1_idx` (`banco`),
  CONSTRAINT `fk_tbl_beneficiario_tbl_banco1` FOREIGN KEY (`banco`) REFERENCES `tbl_banco` (`id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
DROP TABLE IF EXISTS `tbl_moneda` CASCADE;
...
CREATE TABLE `tbl_partida_detalle` (
  ...
  CONSTRAINT `fk_tbl_partida_detalle_tbl_partida1` FOREIGN KEY (`partida`) REFERENCES `tbl_partida` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
INSERT INTO `tbl_tipo_partida` ( ID, NOMBRE, DESCRIP, EDITABLE ) VALUES
 ( 1, 'CHE', 'cheque', 0 ),
 ( 2, 'DEP', 'deposito', 0 ),
 ( 3, 'NCR', 'nota de credito', 0 ),
 ( 4, 'NDB', 'nota de debito', 0 ),
 ( 5, 'PAP', 'partida de apertura de periodo', 0 ),
 ( 6, 'PCI', 'partida de cierre de ejercicio', 0 ),
 ( 7, 'POL', 'partida de poliza', 0 ),
 ( 8, 'LIQ', 'partida de liquidación', 0 );
